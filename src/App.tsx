...
function App() {
  ...
  const [activeApp, setActiveApp] = useState(() => localStorage.getItem('activeApp') || 'ai-chat')
    ...
    setChats((prev) =>
      prev.map((c) =>
        c.id === chatId
          ? {
              ...c,
              title: c.title === '新しいチャット' ? message.slice(0, 20) : c.title,
            ...
            setChats((prev) =>
              prev.map((c) =>
                c.id === chatId
                  ? {
                      ...c,
                      messages: [...c.messages, { role: 'assistant', content: step.content }],
                    }
                  : c,
              ),
            )
  ...
  return (
    <div className="app">
      <AppSidebar activeApp={activeApp} onSelectApp={handleSelectApp} />
      {activeApp === 'ai-chat' ? (
        <>
          {sidebarOpen && (
          ...
          <div
            className={`header ${sidebarOpen ? '' : 'full-width'}`}
            style={sidebarOpen ? { left: sidebarWidth + 72 } : {}}
          >
            {!sidebarOpen && (
          ...
          <div
            className={`chat-area ${sidebarOpen ? '' : 'full-width'}`}
            style={sidebarOpen ? { left: sidebarWidth + 72 } : {}}
          >
            ...
            {currentStep && (
              <div className="message message-step">
                 ...
                 */}
                <TypingText
                  key={`${currentStep.type}-${currentStep.content}`}
                  text={`[${currentStep.type}] ${currentStep.content}`}
                  speed={15}
                />
              </div>
            )}
          ...
          <div
            className={`input-area ${sidebarOpen ? '' : 'full-width'}`}
            style={sidebarOpen ? { left: sidebarWidth + 72 } : {}}
          >
            <textarea
              value={message}
